...
def taskdetails():
    ...
    return render_template('taskdetails.html',query=query)
...
def search():
    if request.method=="POST":
        ...
        task=Tasks.query.filter_by(rollno=rollno).all()
        return render_template('search.html',bio=bio,attend=attend,task=task)
...
@login_required
def tdelete(id):
    post=Tasks.query.filter_by(id=id).first()
    db.session.delete(post)
    db.session.commit()
    # db.engine.execute(f"DELETE FROM `student` WHERE `student`.`id`={id}")
    flash("Slot Deleted Successful","danger")
...
@app.route("/taskedit/<string:id>",methods=['POST','GET'])
@login_required
def taskedit(id):
    # dept=db.engine.execute("SELECT * FROM `department`")    
    if request.method=="POST":
        ...
        return redirect('/taskdetails')
    dept=Department.query.all()
    posts=Tasks.query.filter_by(id=id).first()
    return render_template('taskedit.html',posts=posts,dept=dept)
...
def addtask():
    ...
    if request.method=="POST":
        ...
        rollno=request.form.get('rollno')
        # query=db.engine.execute(f"INSERT INTO `student` (`rollno`,`sname`,`sem`,`gender`,`branch`,`email`,`number`,`address`) VALUES ('{rollno}','{sname}','{sem}','{gender}','{branch}','{email}','{num}','{address}')")
        query=Tasks(taskname=taskname,description=description,assignee=assignee,duedate=duedate,priority=priority,status=status,rollno=rollno)
        db.session.add(query)
        db.session.commit()
    ...
    return render_template('task.html',dept=dept)
